...
class False extends React.Component{
  ...
  calculate = async () => {
    try {
      ...
      let x1 = (xlcal*fxr - xrcal*fxl)/(fxr - fxl);
      let fx1 = evaluate(equation,{[variable]:x1});
      let x1old = 0 , err;
      let history = [];
      let iter = 0;
      do {
        x1 = (xlcal*fxr - xrcal*fxl)/(fxr - fxl);
        err = Math.abs((x1 - x1old)/x1);
        ...
        fx1 = evaluate(equation,{[variable]:x1});
        iter++;
      } while (err > tol);
            ...
            {/* <MathJaxContext>
                <div className="w-full max-w-4xl px-4">
                    <div className="bg-white-50 rounded-2xl shadow-md mt-1 p-4 text-center">
                        <MathJax className="text-3xl">{"\\[x_{n+1}=" +((equation && equation.replaceAll("x", "x_n")) || "\\;...\\;")+ "\\]"}</MathJax>
                    </div>
                </div>
            </MathJaxContext> */}
          </div>
