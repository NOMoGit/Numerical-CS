...
class Conjugrade extends React.Component{
    constructor(props){
        super(props);
        this.state = {
            n: 4,
    ...
    Matvec = (mata,matx,maty) => {
        try {
            const {n} =this.state;
            for(let i = 0 ; i < n ; i++){
                maty[i] = 0;
                for(let j = 0 ; j < n ; j++){
                    maty[i] += mata[i][j] * matx[j];
                }
            }
        } catch (error) {
            alert("Matvec");
    ...
    calculate = () =>{
        try {
            ...
            let matr = Array(size).fill(0);
            let matp = Array(size).fill(0);
            let matAp = Array(size).fill(0);
            ...
            for(let i = 0 ; i < size ; i++){
                ...
                matr[i] = parseFloat(matrixB[i]);
                matp[i] = parseFloat(matrixB[i]);
            }
            let resultmat = Array(size).fill(0);
            let rr = this.Dot(matr,matr);
            do {
                iter++;
                this.Matvec(matA,matp,matAp);
                let alpha = rr / this.Dot(matp,matAp);
                for(let i = 0 ; i < size ; i++){
                    matX[i] += alpha * matp[i];
                }
                for(let i = 0 ; i < size ; i++){
                    matr[i] -= alpha * matAp[i];
                }
                let rrnew = this.Dot(matr,matr);
                err = Math.sqrt(rrnew);

                let beta = rrnew / rr;
                for(let i = 0 ; i < n ; i++){
                    matp[i] = matr[i] + beta * matp[i];
                }
                rr = rrnew;
                iterationError = err
            } while (err >= tol);
            this.setState({result:matX,error:iterationError.toFixed(10)});
        } catch (error) {
            alert("Error Catch Calculate");
        }
    }
    render(){
        const {n , matrixA , matrixB , matrixX , tolerance , result , error} = this.state;
        return(
            <div className="min-h-screen bg-gray-to-br from-blue-50 via-indigo-50 to-purple-50 py-12 px-4">
                <div className="max-w-7xl mx-auto">
                    ...
                    <div className="sm:flex-row items-center justify-center gap-10   pb-6  max-w-2xl mx-auto ">
                        <div className="flex flex-col sm:flex-row items-center justify-center  max-w-2xl mx-auto">
                            <h2 className="text-2xl font-bold text-center  text-blue-700">{"{X}"}</h2>
                        </div>
                    ...
                    <div className=" flex flex-col lg:flex-row gap-6 items-center justify-center mb-8">
                        ...
                        <div className="flex flex-col sm:flex-row items-center justify-center gap-10 mb-8  p-6  max-w-2xl mx-auto">
                            ...
                            <div className="bg-white p-6 rounded-2xl shadow-lg">
                                <h2 className="text-2xl font-bold text-center mb-4 text-blue-700">{"{X}"}</h2> 
                                <div className='flex flex-col gap-2'>
                ...
                {result.length > 0 && (
                    <div className="bg-white p-8 rounded-2xl shadow-lg max-w-2xl mx-auto ">
                        <h2 className="text-3xl font-bold text-center mb-6 text-transparent bg-blue-to-r from-blue-600 to-blue-600 bg-clip-text">Solution</h2>
                        <div className="space-y-3">
                            {result.map((r,i) => (
                                    <div 
                                        key={i}
                                        className='bg-gray-to-r border from-gray-50 to-gray-50 p-4 rounded-xl flex items-center justify-between '>
